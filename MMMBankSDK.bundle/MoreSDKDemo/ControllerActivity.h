...
/** 临时方法
 *  @param presentViewContrller 设定当前的viewController
 *                    (临时紧急方法, 当用户需要接入并跳出乾多多SDK页面时,但没有效果,既乾多多插件页面
 *                     没有跳出, 则在调用接口方法前先设置该方法,参数一般为当前的viewController.
 *                     配合+clearPresentViewController方法一起使用. 在调用完接口方法后使用
 *                     +clearPresentViewController方法)
 *
 */
+(void)setPresentViewController:(UIViewController *)presentViewContrller;
+(void)clearPresentViewController;
